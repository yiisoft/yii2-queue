...
Чтобы использовать расширение в своем проекте, необходимо дополнить конфигурацию приложения следующим образом:
...
Способы обработки очереди задач могут отличаться в зависимости от используемого драйвера. В большей
...
Если воркер разрабатывался не на PHP, то необходимо изменить способ сериализации данных, чтобы сообщения 
кодировались доступным воркеру форматом. Например, настроить сериализацию в json можно следующим образом:
...
Очередь инициализирует следующие события:

| Событие                      | Класс события | Когда инициализируется                                             |
|------------------------------|---------------|---------------------------------------------------------------|
...
Например, отложить выполнение задания, которое выбросило специальное исключение:
...
Компонент предоставляет `LogBehavior` для логирования событий, используя
